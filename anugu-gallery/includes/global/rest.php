...
class Anugu_Rest {
	...
	/**
	 * Helper Method to register Anugu gallery Meta
	 *
	 * @since 1.8.5
	 *
	 * @return void
	...
	public function get_gallery_data( $object, $field_name, $request ) {
		...
		$data = apply_filters( 'anugu_gallery_pre_data', $data, $object['id'] );
	...
	public function update_gallery_data( $value, $object, $field_name ) {
		...
		if ( ! is_array( $gallery_data['config'] ) ) {
			$common = new Anugu_Gallery_Common();
		...
		if ( isset( $value['gallery'] ) ) {
			...
			foreach ( (array) $value['gallery'] as $i => $image ) {
				$gallery_data = anugu_gallery_ajax_prepare_gallery_data( $gallery_data, $image['id'] );
			}
		}
	...
	public function get_image_src( $id, $item, $data, $mobile = false, $retina = false ) {
		...
		if ( ! $src && is_int( $id ) ) : // wp_get_attachment_image_src only accepts $id as integer
			...
			if ( ( $this->get_config( 'crop', $data ) && $image_size == 'default' ) || $image_size == 'full' ) {
				...
				$src = apply_filters( 'anugu_gallery_retina_image_src', wp_get_attachment_image_src( $id, 'full' ), $id, $item, $data, $mobile );
				...
				if ( $image_size != 'default' ) {
					...
					$src   = apply_filters( 'anugu_gallery_default_image_src', wp_get_attachment_image_src( $id, $isize ), $id, $item, $data, $mobile );
		...
		if ( $crop || $type == 'mobile' ) {
			...
			$image_to_crop = apply_filters( 'anugu_gallery_crop_image_src', wp_get_attachment_image_src( $id, 'full' ), $id, $item, $data, $mobile );
			// Check if this returned an image.
			if ( ! $image_to_crop ) {
				// Fallback to the $item's image source.
    ...
    public static function get_instance() {
        ...
        if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Anugu_Rest ) ) {
...
$anugu_rest = Anugu_Rest::get_instance();