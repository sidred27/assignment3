...
class Anugu_Gallery_Shortcode {
	...
	public $wp_kses_allowed_html = array(
		...
		'li'     => array(
			'id'                              => array(),
			'class'                           => array(),
			'data-anugu-gallery-image'       => array(),
			'data-anugu-gallery-image-model' => array(),
		),
		'em'     => array(),
		'span'   => array(
			'class' => array(),
    ...
    public function __construct() {
		...
		$this->common = Anugu_Gallery_Common::get_instance();
		$this->is_mobile = anugu_mobile_detect()->isMobile();

        // Register main gallery style.
        wp_register_style( $this->base->plugin_slug . '-style', plugins_url( 'assets/css/anugu.css', $this->base->file ), array(), $this->base->version );
        ...
        wp_register_script( $this->base->plugin_slug . '-script', plugins_url( 'assets/js/min/anugu-min.js', $this->base->file ), array( 'jquery' ), $this->base->version, true );

        // Load hooks and filters.
        add_shortcode( 'anugu-gallery', array( $this, 'shortcode' ) );
        add_filter( 'widget_text', 'do_shortcode' );
        //add_filter( 'anugu_gallery_output_image_attr', array( $this, 'add_image_srcset_attributes' ), 10, 5 );
    ...
    public function shortcode( $atts ) {
        ...
        if ( empty( $atts ) ) {
            ...
            $data = apply_filters( 'anugu_gallery_custom_gallery_data', false, $atts, $post );
        ...
        // [anugu-gallery id="123" limit="10"] would only display 10 images
        ...
        $data = apply_filters( 'anugu_gallery_pre_data', $data, $gallery_id );
        ...
        if ( $this->get_config( 'lazy_loading', $data ) == 1 ) {
            wp_localize_script( $this->base->plugin_slug . '-script', 'anugu_lazy_load', 'true');
            wp_localize_script( $this->base->plugin_slug . '-script', 'anugu_lazy_load_initial', 'false');
            wp_localize_script( $this->base->plugin_slug . '-script', 'anugu_lazy_load_delay', (string) $lazy_loading_delay);
        }
        ...
        do_action( 'anugu_gallery_before_output', $this->gallery_data );

        // Apply a filter before starting the gallery HTML.
        $gallery = apply_filters( 'anugu_gallery_output_start', $gallery, $this->gallery_data );

        // Build out the gallery HTML.
        $gallery .= '<div id="anugu-gallery-wrap-' . sanitize_html_class( $this->gallery_data['id'] ) . '" class="' . $this->get_gallery_classes( $this->gallery_data ) . '" itemscope itemtype="https://schema.org/ImageGallery">';
            $gallery  = apply_filters( 'anugu_gallery_output_before_container', $gallery, $this->gallery_data );
            ...
            // add justified CSS?
            $extra_css = 'anugu-gallery-justified-public';
            $row_height = false;
            ...
            $gallery .= '<div' .  $opacity_insert . ' data-row-height="'.$row_height.'" data-gallery-theme="'.$justified_gallery_theme.'" id="anugu-gallery-' . sanitize_html_class( $this->gallery_data['id'] ) . '" class="anugu-gallery-public '.$extra_css.' anugu-gallery-' . sanitize_html_class( $this->get_config( 'columns', $this->gallery_data ) ) . '-columns anugu-clear' . ( $this->get_config( 'isotope', $this->gallery_data ) ? ' anugutope' : '' ) . ( $this->get_config( 'css_animations', $this->gallery_data ) ? ' anugu-gallery-css-animations' : '' ) . '" data-anugu-columns="' . $this->get_config( 'columns', $this->gallery_data ) . '">';
            ...
            $gallery  = apply_filters( 'anugu_gallery_output_after_container', $gallery, $this->gallery_data );

        $gallery .= '</div>';
        $gallery  = apply_filters( 'anugu_gallery_output_end', $gallery, $this->gallery_data );
        ...
        if ( $this->get_config( 'mobile', $this->gallery_data ) ) {
            remove_filter( 'anugu_gallery_output_image_attr', array( $this, 'mobile_image' ), 999, 4 );
        }
        ...
        return apply_filters( 'anugu_gallery_output', $gallery, $this->gallery_data );
    ...
    */
    public function generate_gallery_item_markup( $gallery, $data, $item, $id, $i ) {
        ...
        $html5_attribute     = ( ( $this->get_config( 'html5', $data ) == '1' ) ? 'data-anugubox-group' : 'rel' );
        ...
        $item     = apply_filters( 'anugu_gallery_output_item_data', $item, $id, $data, $i );
        ...
        $image_src_retina = apply_filters( 'anugu_gallery_output_item_src_retina', $this->get_image_src( $id, $item, $data, false, true ), $item, $id, $data, $i, $this->is_mobile );
        ...
        $gallery  = apply_filters( 'anugu_gallery_output_before_item', $gallery, $id, $item, $data, $i );
        ...
        $output   = '<div id="anugu-gallery-item-' . sanitize_html_class( $id ) . '" class="' . $this->get_gallery_item_classes( $item, $i, $data ) . '" style="padding-left: ' . $padding . 'px; padding-bottom: ' . $this->get_config( 'margin', $data ) . 'px; padding-right: ' . $padding . 'px;" ' . apply_filters( 'anugu_gallery_output_item_attr', '', $id, $item, $data, $i ) . ' itemscope itemtype="https://schema.org/ImageObject">';

        $output .= '<div class="anugu-gallery-item-inner">';
        $output  = apply_filters( 'anugu_gallery_output_before_link', $output, $id, $item, $data, $i );

        // Top Left box.

        $css_class = false; // no css classes yet
        $css_class = apply_filters( 'anugu_gallery_output_dynamic_position_css', $css_class, $output, $id, $item, $data, $i, 'top-left' );

        $output .= '<div class="anugu-gallery-position-overlay ' . $css_class . ' anugu-gallery-top-left">';
        $output  = apply_filters( 'anugu_gallery_output_dynamic_position', $output, $id, $item, $data, $i, 'top-left' );
        $output .= '</div>';

        // Top Right box.

        $css_class = false; // no css classes yet
        $css_class = apply_filters( 'anugu_gallery_output_dynamic_position_css', $css_class, $output, $id, $item, $data, $i, 'top-right' );

        $output .= '<div class="anugu-gallery-position-overlay ' . $css_class . ' anugu-gallery-top-right">';
        $output  = apply_filters( 'anugu_gallery_output_dynamic_position', $output, $id, $item, $data, $i, 'top-right' );
        $output .= '</div>';

        // Bottom Left box.

        $css_class = false; // no css classes yet
        $css_class = apply_filters( 'anugu_gallery_output_dynamic_position_css', $css_class, $output, $id, $item, $data, $i, 'bottom-left' );

        $output .= '<div class="anugu-gallery-position-overlay ' . $css_class . ' anugu-gallery-bottom-left">';
        $output  = apply_filters( 'anugu_gallery_output_dynamic_position', $output, $id, $item, $data, $i, 'bottom-left' );
        $output .= '</div>';

        // Bottom Right box.
        $css_class = false; // no css classes yet
        $css_class = apply_filters( 'anugu_gallery_output_dynamic_position_css', $css_class, $output, $id, $item, $data, $i, 'bottom-right' );

        $output .= '<div class="anugu-gallery-position-overlay ' . $css_class . ' anugu-gallery-bottom-right">';
        $output  = apply_filters( 'anugu_gallery_output_dynamic_position', $output, $id, $item, $data, $i, 'bottom-right' );
        $output .= '</div>';
        ...
        $create_link = apply_filters( 'anugu_gallery_create_link', $create_link, $data, $id, $item, $i, $this->is_mobile );

        if ( $create_link ) {
            $output .= '<a href="' . esc_url( $item['link'] ) . '" class="' . ( ( isset($item['link_new_window']) && $item['link_new_window'] == 1 ) ? '' : 'anugu-gallery-' . sanitize_html_class( $data['id'] ) . ' ' ) . 'anugu-gallery-link" ' . $html5_attribute . '="anugugallery' . sanitize_html_class( $data['id'] ) . '" title="' . $title . '" data-anugu-caption="' . $caption . '" data-anugu-retina="' . ( isset( $item['lightbox_retina_image'] ) ? $item['lightbox_retina_image'] : '' ) . '" data-thumbnail="' . esc_url( $item['thumb'] ) . '"' . ( ( isset($item['link_new_window']) && $item['link_new_window'] == 1 ) ? ' target="_blank"' : '' ) . ' ' . apply_filters( 'anugu_gallery_output_link_attr', '', $id, $item, $data, $i ) . ' itemprop="contentUrl">';
        }

        $output  = apply_filters( 'anugu_gallery_output_before_image', $output, $id, $item, $data, $i );

        $gallery_theme = $this->get_config( 'columns', $data ) == 0 ? ' anugu-' . $this->get_config( 'justified_gallery_theme', $data ) : '';
        ...
        $anugu_lazy_load = $this->get_config( 'lazy_loading', $data ) == 1 ? 'anugu-lazy' : '';
        ...
        if ( $this->get_config( 'columns', $data ) == 0 ) {
            ...
            $output_item = '<img id="anugu-gallery-image-' . sanitize_html_class( $id ) . '" class="anugu-gallery-image anugu-gallery-image-' . $i . $gallery_theme . ' '.$anugu_lazy_load.'" data-anugu-index="' . $i . '" src="' . esc_url( $output_src ) . '"' . ( $this->get_config( 'dimensions', $data ) ? ' width="' . $this->get_config( 'crop_width', $data ) . '" height="' . $this->get_config( 'crop_height', $data ) . '"' : '' ) . ' data-anugu-src="' . esc_url( $output_src ) . '" data-anugu-gallery-id="' . $data['id'] . '" data-anugu-item-id="' . $id . '" data-anugu-caption="' . wp_strip_all_tags( htmlspecialchars( $title ) ) . '" alt="' . esc_attr( $item['alt'] ) . '" title="' . strip_tags( htmlspecialchars( $item['title'] ) ) . '" ' . apply_filters( 'anugu_gallery_output_image_attr', '', $id, $item, $data, $i ) . ' itemprop="thumbnailUrl" data-anugu-srcset="' . esc_url( $output_src ) . ' 400w,' . esc_url( $output_src ) . ' 2x" data-anugu-width="'.$output_width.'" data-anugu-height="'.$output_height.'" srcset="' . ( ( $anugu_lazy_load ) ? 'data:image/gif;base64,R0lGODlhAQABAIAAAP///////yH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==' : esc_url( $image_src_retina ) . ' 2x' ) . '" data-safe-src="'. ( ( $anugu_lazy_load ) ? 'data:image/gif;base64,R0lGODlhAQABAIAAAP///////yH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==' : esc_url( $output_src ) ) . '" />';
            ...
            if ( $anugu_lazy_load ) {
                ...
                $output_item .= '<div class="anugu-lazy" data-test-width="'.$output_width.'" data-test-height="'.$output_height.'" style="padding-bottom:'.$padding_bottom.'%;">';

            }

            $output_item .= '<img id="anugu-gallery-image-' . sanitize_html_class( $id ) . '" class="anugu-gallery-image anugu-gallery-image-' . $i . $gallery_theme . '" data-anugu-index="' . $i . '" src="' . esc_url( $output_src ) . '"' . ( $this->get_config( 'dimensions', $data ) ? ' width="' . $this->get_config( 'crop_width', $data ) . '" height="' . $this->get_config( 'crop_height', $data ) . '"' : '' ) . ' data-anugu-src="' . esc_url( $output_src ) . '" data-anugu-gallery-id="' . $data['id'] . '" data-anugu-item-id="' . $id . '" data-anugu-caption="' . $caption . '" alt="' . esc_attr( $item['alt'] ) . '" title="' . wp_strip_all_tags( htmlspecialchars( $title ) ) . '" ' . apply_filters( 'anugu_gallery_output_image_attr', '', $id, $item, $data, $i ) . ' itemprop="thumbnailUrl" data-anugu-srcset="' . esc_url( $output_src ) . ' 400w,' . esc_url( $output_src ) . ' 2x" srcset="' . ( ( $anugu_lazy_load ) ? 'data:image/gif;base64,R0lGODlhAQABAIAAAP///////yH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==' : esc_url( $image_src_retina ) . ' 2x' ) . '" />';
            ...
            if ( $anugu_lazy_load ) {
                ...
                $output_item = apply_filters( 'anugu_gallery_output_image', $output_item, $id, $item, $data, $i );

                // Add image to output
                $output .= $output_item;
                $output  = apply_filters( 'anugu_gallery_output_after_image', $output, $id, $item, $data, $i );

            if ( $create_link ) {
                $output .= '</a>';
            }

            $output  = apply_filters( 'anugu_gallery_output_after_link', $output, $id, $item, $data, $i );
            $output .= '</div>';

        $output .= '</div>';
        $output  = apply_filters( 'anugu_gallery_output_single_item', $output, $id, $item, $data, $i );

        // Append the image to the gallery output
        $gallery .= $output;

        // Filter the output before returning
        $gallery  = apply_filters( 'anugu_gallery_output_after_item', $gallery, $id, $item, $data, $i );
    ...
    */
    public function add_stylesheet_property_attribute( $tag ) {
        ...
        // If the <link> stylesheet is any Anugu-based stylesheet, add the property attribute
        if ( strpos( $tag, "id='anugu-" ) !== false ) {
    ...
    public function maybe_sort_gallery( $data, $gallery_id ) {
        ...
        switch ( $sorting_method ) {
            ...
            */
            default:
                $data = apply_filters( 'anugu_gallery_sort_gallery', $data, $sorting_method, $gallery_id );
                break;
    ...
    */
    public function description( $gallery, $data ) {
        ...
        $gallery .= '<div class="anugu-gallery-description anugu-gallery-description-above" style="padding-bottom: ' . $this->get_config( 'margin', $data ) . 'px;">';
            $gallery  = apply_filters( 'anugu_gallery_output_before_description', $gallery, $data );
            ...
            $gallery  = apply_filters( 'anugu_gallery_output_after_description', $gallery, $data );
        $gallery .= '</div>';
    ...
    public function gallery_init() {
        ...
        // anugu_galleries stores all Fancybox instances
        // anugu_isotopes stores all Isotope instances
        // anugu_isotopes_config stores Isotope configs for each Gallery
        ?>
        ...
        <script type="text/javascript">
            ...
            var anugu_galleries = [],
                anugu_gallery_images = [],
                anugu_isotopes = [],
                anugu_isotopes_config = [];

            jQuery(document).ready(function($){

                <?php
                do_action( 'anugu_gallery_api_start_global' );
                foreach ( $this->data as $data ) {
                    ...
                    do_action( 'anugu_gallery_api_start', $data );

                    // Define container
                    ?>
                    var anugu_container_<?php echo $data['id']; ?> = '';

                function anugu_album_lazy_load_image( $id ) {

                    <?php if ( $this->get_config( 'lazy_loading', $data ) ) { ?>

				var responsivelyLazy = window.responsivelyLazy;

                    responsivelyLazy.run('#anugu-gallery-'+ $id);

                    <?php } else { ?>
                        /* to do: enable ANUGU DEBUG to display this and other errors */
                    ...
                    ?>
                        ...
                        $('#anugu-gallery-<?php echo $data["id"]; ?>').anuguJustifiedGallery({
                        ...
                        $('#anugu-gallery-<?php echo $data["id"]; ?>').justifiedGallery().on('jg.complete', function (e) {

                            anugu_album_lazy_load_image(<?php echo $data['id']; ?>);
                        ...
                        $( document ).on( "anugu_pagination_ajax_load_completed", function() {
                            $('#anugu-gallery-<?php echo $data["id"]; ?>').justifiedGallery().on('jg.complete', function (e) {
                        ...
                        $('#anugu-gallery-<?php echo $data["id"]; ?>').on('jg.complete', function (e) {
                            if( navigator.userAgent.match(/msie/i) || $.browser.msie || navigator.appVersion.indexOf('Trident/') > 0 ) {
                                $('#anugu-gallery-<?php echo $data["id"]; ?> img').each(function() {
                                ...
                                $('#anugu-gallery-<?php echo $data["id"]; ?> img').hover(
                                    function() {
                                ...
                                /*$('#anugu-gallery-<?php echo $data["id"]; ?> img').each(function() {
                                    $(this).addClass('anugu-<?php echo $gallery_theme; ?>');
                                });*/

                                $('#anugu-gallery-<?php echo $data["id"]; ?> img').hover(
                                    function() {
                                        $(this).removeClass('anugu-<?php echo $gallery_theme; ?>');
                                    },
                                    function() {
                                        $(this).addClass('anugu-<?php echo $gallery_theme; ?>');
                                    }
                        ...
                        $('#anugu-gallery-<?php echo $data["id"]; ?>').css('opacity', '1');
                    ...
                    if ( $this->get_config( 'columns', $data ) > 0 && $this->get_config( 'isotope', $data ) ) {
                        ...
                        ?>
                        anugu_isotopes_config['<?php echo $data['id']; ?>'] = {
                            <?php do_action( 'anugu_gallery_api_anugutope_config', $data ); ?>
                            itemSelector: '.anugu-gallery-item',
                            <?php
                            // If columns = 0, use fitRows
                            // if ( $this->get_config( 'columns', $data ) > 0 ) {
                                ?>
                                masonry: {
                                    columnWidth: '.anugu-gallery-item'
                                }
                                <?php /*
                            } else {
                                ?>
                        ...
                        // Initialize Isotope
                        ?>
                        anugu_isotopes['<?php echo $data['id']; ?>'] = anugu_container_<?php echo $data['id']; ?>
                                                                    = $('#anugu-gallery-<?php echo $data['id']; ?>').anugutope(anugu_isotopes_config['<?php echo $data['id']; ?>']);

                        $('#anugu-gallery-<?php echo $data["id"]; ?>').on( 'layoutComplete',
                        ...
                        ?>
                        anugu_isotopes['<?php echo $data['id']; ?>'].anuguImagesLoaded()
                            .done(function() {
                                anugu_isotopes['<?php echo $data['id']; ?>'].anugutope('layout');
                            })
                            .progress(function() {
                                anugu_isotopes['<?php echo $data['id']; ?>'].anugutope('layout');
                            });
                        <?php
                        do_action( 'anugu_gallery_api_anugutope', $data );
                    }
                    // Isotope: End
                    ...
                    if ( $this->get_config( 'css_animations', $data ) ) {
                        ...
                        $opacity = ( $opacity / 100 );
                        ?>
                        anugu_container_<?php echo $data['id']; ?> = $('#anugu-gallery-<?php echo $data['id']; ?>').anuguImagesLoaded( function() {
                            $('.anugu-gallery-item img').fadeTo( 'slow', <?php echo $opacity; ?> );
                        });
                        <?php
                    }
                    // CSS Animations: End
                    ...
                    if ( $this->get_config( 'lightbox_enabled', $data ) ) {
                        ...
                        // Those Addons can populate this array now which will tell anugubox which images to use.
                        $lightbox_images = apply_filters( 'anugu_gallery_lightbox_images', false, $data );
                        $title_fb1 = apply_filters( 'anugu_gallery_title_display', $this->get_config( 'title_display', $data ), $data );
                        if ( 'float_wrap' == $title_fb1 || 'float' == $title_fb1 ) {
	                        $title_fb1 = 'float';
                        }

                        ?>
                        anugu_gallery_options = {
	                        // All of these options are for Anugu Lite with Fancybox 1 to try and match Fancybox 2.
	                        padding: <?php echo in_array( $this->get_config( 'lightbox_theme', $data ), array( 'base_dark' ) ) ? '4' : '15'; ?>,
	                        cyclic: true,
	                        titlePosition: '<?php echo $title_fb1; ?>',
	                        // End options for Anugu Lite with Fancybox 1.

                            <?php do_action( 'anugu_gallery_api_config', $data ); // Depreciated ?>
                            <?php do_action( 'anugu_gallery_api_anugubox_config', $data ); ?>
                            <?php if ( ! $this->get_config( 'keyboard', $data ) ) : ?>
                            keys: 0,
                            <?php endif; ?>
                            margin: <?php echo apply_filters( 'anugubox_margin', 60, $data ); ?>,
                            arrows: <?php echo in_array( $this->get_config( 'lightbox_theme', $data ), array( 'base_dark' ) ) ? 'true' : $this->get_config( 'arrows', $data ); ?>,
                            aspectRatio: <?php echo apply_filters( 'anugubox_aspect', $this->get_config( 'aspect', $data ), $data ); ?>,
                            loop: <?php echo apply_filters( 'anugubox_loop', $this->get_config( 'loop', $data ), $data ); ?>,
                            ...
                            if ( in_array( $lightbox_open_close_effect, $lightbox_standard_effects ) ) {
                                ...
                                openSpeed: <?php echo apply_filters( 'anugubox_open_speed', 500, $data ); ?>,
                                closeSpeed: <?php echo apply_filters( 'anugubox_close_speed', 500, $data ); ?>,
                                <?php
                            }
                            ...
                            if ( in_array( $lightbox_transition_effect, $lightbox_standard_effects ) ) {
                                ...
                                nextSpeed: <?php echo apply_filters( 'anugubox_next_speed', 600, $data ); ?>,
                                prevSpeed: <?php echo apply_filters( 'anugubox_prev_speed', 600, $data ); ?>,
                                <?php
                            }
                            ?>
                            tpl: {
                                wrap     : '<?php echo $this->get_lightbox_template( $data ); ?>',
                                image    : '<img class="anugubox-image" src="{href}" alt="" data-anugu-title="" data-anugu-caption="" data-anugu-index="" data-anugu-data="" />',
                                iframe   : '<iframe id="anugubox-frame{rnd}" name="anugubox-frame{rnd}" class="anugubox-iframe" frameborder="0" vspace="0" hspace="0" allowtransparency="true" wekitallowfullscreen mozallowfullscreen allowfullscreen></iframe>',
                                error    : '<p class="anugubox-error"><?php echo __( 'The requested content cannot be loaded.<br/>Please try again later.</p>', 'anugu-gallery-lite' ); ?>',
                                closeBtn : '<a title="<?php echo __( 'Close', 'anugu-gallery-lite' ); ?>" class="anugubox-item anugubox-close" href="#"></a>',
                                next     : '<a title="<?php echo __( 'Next', 'anugu-gallery-lite' ); ?>" class="anugubox-nav anugubox-next anugubox-arrows-<?php echo $this->get_config( 'arrows_position', $data ); ?>" href="#"><span></span></a>',
                                prev     : '<a title="<?php echo __( 'Previous', 'anugu-gallery-lite' ); ?>" class="anugubox-nav anugubox-prev anugubox-arrows-<?php echo $this->get_config( 'arrows_position', $data ); ?>" href="#"><span></span></a>'
                                <?php do_action( 'anugu_gallery_api_templates', $data ); ?>
                            },
                            helpers: {
                                <?php
                                do_action( 'anugu_gallery_api_helper_config', $data );
                                // Grab title display
                                ...
                                ?>
                                title: {
                                    <?php do_action( 'anugu_gallery_api_title_config', $data ); ?>
                            ...
                            <?php do_action( 'anugu_gallery_api_config_callback', $data ); ?>
                            beforeLoad: function(){
                                ...
                                ?>
                                this.title = $(this.element).attr('data-anugu-caption');
                                <?php
                                ...
                                <?php do_action( 'anugu_gallery_api_before_load', $data ); ?>
                            },
                            afterLoad: function(){
                                $('anugubox-overlay-fixed').on({
                                    'touchmove' : function(e){
                                        e.preventDefault();
                                    }
                                });

                                <?php do_action( 'anugu_gallery_api_after_load', $data ); ?>
                            },
                            beforeShow: function(){
                                $(window).on({
                                    'resize.anugubox' : function(){
                                        $.anugubox.update();
                                    }
                                });
                                ...
                                if ( typeof this.element === 'undefined' ) {
                                    ...
                                    ?>
                                    var gallery_id = this.element.find('img').data('anugu-gallery-id');
                                    var gallery_item_id = this.element.find('img').data('anugu-item-id');
                                    var alt = this.element.find('img').attr('alt');
                                    var title = this.element.find('img').parent().attr('title');
                                    var caption = this.element.find('img').parent().data('anugu-caption');
                                    var retina_image = this.element.find('img').parent().data('anugu-retina');
                                    var index = this.element.find('img').data('anugu-index');
                                }

                                <?php
                                // Set alt, data-anugu-title, data-anugu-caption and data-anugu-index attributes on Lightbox image
                                ?>
                                this.inner.find('img').attr('alt', alt)
                                                      .attr('data-anugu-gallery-id', gallery_id)
                                                      .attr('data-anugu-item-id', gallery_item_id)
                                                      .attr('data-anugu-title', title)
                                                      .attr('data-anugu-caption', caption)
                                                      .attr('data-anugu-index', index);
                                ...
                                <?php do_action( 'anugu_gallery_api_before_show', $data ); ?>
                            },
                            onStart: function(){
                                <?php ?>
                                $('#anugubox-wrap, #anugubox-wrap #anugubox-left, #anugubox-wrap #anugubox-right').swipe( {
                                    excludedElements:"label, button, input, select, textarea, .noSwipe",
                                    swipe: function(event, direction, distance, duration, fingerCount, fingerData) {
                                        if (direction === 'left') {
                                            $.anugubox.next(direction);
                                        } else if (direction === 'right') {
                                            $.anugubox.prev(direction);
                                ...
                                if ( $this->get_config( 'title_display', $data ) == 'float_wrap' ) {
                                    ...
                                    if ( typeof this.helpers.title !== 'undefined' ) {
                                        if ( ! $( 'div.anugubox-title' ).hasClass( 'anugubox-title-text-wrap' ) ) {
                                            $( 'div.anugubox-title' ).addClass( 'anugubox-title-text-wrap' );
                                        }
                                    }
                                    <?php
                                }

                                do_action( 'anugu_gallery_api_after_show', $data ); ?>
                            },
                            beforeClose: function(){
                                <?php do_action( 'anugu_gallery_api_before_close', $data ); ?>
                            },
                            afterClose: function(){
                                $(window).off('resize.anugubox');
                                <?php do_action( 'anugu_gallery_api_after_close', $data ); ?>
                            },
                            onUpdate: function(){
                                <?php
                                if ( $this->get_config( 'toolbar', $data ) ) : ?>
                                var anugu_buttons_<?php echo $data['id']; ?> = $('#anugubox-buttons li').map(function(){
                                    return $(this).width();
                                }).get(),
                                    anugu_buttons_total_<?php echo $data['id']; ?> = 0;
                                $.each(anugu_buttons_<?php echo $data['id']; ?>, function(i, val){
                                    anugu_buttons_total_<?php echo $data['id']; ?> += parseInt(val, 10);
                                });

                                anugu_buttons_total_<?php echo $data['id']; ?> += 1;

                                $('#anugubox-buttons ul').width(anugu_buttons_total_<?php echo $data['id']; ?>);
                                $('#anugubox-buttons').width(anugu_buttons_total_<?php echo $data['id']; ?>).css('left', ($(window).width() - anugu_buttons_total_<?php echo $data['id']; ?>)/2);
                                <?php endif; ?>

                                <?php do_action( 'anugu_gallery_api_on_update', $data ); ?>
                            },
                            onCancel: function(){
                                <?php do_action( 'anugu_gallery_api_on_cancel', $data ); ?>
                            },
                            onPlayStart: function(){
                                <?php do_action( 'anugu_gallery_api_on_play_start', $data ); ?>
                            },
                            onPlayEnd: function(){
                                <?php do_action( 'anugu_gallery_api_on_play_end', $data ); ?>
                            }
                        };
                        <?php
                        ...
                        if ( ! $lightbox_images ) {
                            ...
                            ?>
                            anugu_galleries['<?php echo $data['id']; ?>'] = $('.anugu-gallery-<?php echo $data['id']; ?>').anugubox( anugu_gallery_options );
                            <?php

                        } else {
                            // Use images from $lightbox_images
                            add_filter( 'anugu_minify_strip_double_forward_slashes', '__return_false' );
                            ?>
                            anugu_gallery_images[<?php echo $data['id']; ?>] = [];
                            <?php
                            ...
                            foreach ( $lightbox_images as $image_id => $image ) {
                                ...
                                ?>
                                anugu_gallery_images[<?php echo $data['id']; ?>].push({
                                    ...
                                    <?php do_action( 'anugu_gallery_api_lightbox_image_attributes', $image, $image_id, $lightbox_images, $data ); ?>
                                });
                                <?php
                                $count++;
                            }

                            // Open anugubox when an image is clicked, telling anugubox which images are available to it.
                            ?>
                            anugu_galleries['<?php echo $data['id']; ?>'] = $('.anugu-gallery-<?php echo $data['id']; ?>').anugubox( anugu_gallery_options, anugu_gallery_images[<?php echo $data['id']; ?>] );
                            $('#anugu-gallery-wrap-<?php echo $data['id']; ?>').on('click', 'a.anugu-gallery-link', function(e) {
                                e.preventDefault();
                                e.stopPropagation();
                                $.anugubox.close();
                                if ( $('#anugu-gallery-wrap-<?php echo $data['id']; ?> div.anugu-pagination').length > 0 ) { /* this exists, perhaps pagination doesn't display nav because it's only one page? */
                                    var anugubox_page = ( $('#anugu-gallery-wrap-<?php echo $data['id']; ?> div.anugu-pagination').data('page') - 1 );
                                } else {
                                    var anugubox_page = 0;
                                }

                                if ( $('#anugu-gallery-wrap-<?php echo $data['id']; ?> div.anugu-pagination').length > 0 ) { /* this exists, perhaps pagination doesn't display nav because it's only one page? */
                                    var anugubox_per_page = $('#anugu-gallery-wrap-<?php echo $data['id']; ?> div.anugu-pagination').data('per-page');
                                } else {
                                    var anugubox_per_page = $('.anugu-gallery-image').length;
                                }

                                var anugubox_index = ( Number($('img', $(this)).data('anugu-index')) - 1 );
                                anugu_gallery_options.index = ((anugubox_page * anugubox_per_page) + anugubox_index);
                                anugu_galleries['<?php echo $data['id']; ?>'] = $.anugubox( anugu_gallery_images[<?php echo $data['id']; ?>], anugu_gallery_options );
                            });
                            <?php
                        }

                        do_action( 'anugu_gallery_api_lightbox', $data );
                        // Fancybox: End
                    }

                    do_action( 'anugu_gallery_api_end', $data );
                } // foreach

                do_action( 'anugu_gallery_api_end_global', $this->data );
    ...
    public function get_gallery_classes( $data ) {
        ...
        // Set default class.
        $classes   = array();
        $classes[] = 'anugu-gallery-wrap';

        // Add custom class based on data provided.
        $classes[] = 'anugu-gallery-theme-' . $this->get_config( 'gallery_theme', $data );
        $classes[] = 'anugu-lightbox-theme-' . $this->get_config( 'lightbox_theme', $data );
        ...
        if ( $this->get_config( 'rtl', $data ) ) {
            $classes[] = 'anugu-gallery-rtl';
        }

        // Allow filtering of classes and then return what's left.
        $classes = apply_filters( 'anugu_gallery_output_classes', $classes, $data );
    ...
    public function get_gallery_item_classes( $item, $i, $data ) {
        ...
        // Set default class.
        $classes   = array();
        $classes[] = 'anugu-gallery-item';
        $classes[] = 'anugutope-item';
        $classes[] = 'anugu-gallery-item-' . $i;

        // If lazy load exists, add that
        if ( isset( $data['config']['lazy_loading'] ) && $data['config']['lazy_loading'] ) {
            $classes[] = 'anugu-lazy-load';
        }

        // Allow filtering of classes and then return what's left.
        $classes = apply_filters( 'anugu_gallery_output_item_classes', $classes, $item, $i, $data );
        return trim( implode( ' ', array_map( 'trim', array_map( 'sanitize_html_class', array_unique( $classes ) ) ) ) );
	...
	public function get_image_src( $id, $item, $data, $mobile = false, $retina = false ) {
		...
		if ( !$src ) :
			...
			if ( $image_size != 'default' && ! $retina ) {
				// If image size is anugu_gallery_random, get a random image size.
				if ( $image_size == 'anugu_gallery_random' ) {
				...
				if ( ! $retina ){
					...
					$src = apply_filters( 'anugu_gallery_retina_image_src', wp_get_attachment_image_src( $id, $isize ), $id, $item, $data, $this->is_mobile );

				}else{
					$src = apply_filters( 'anugu_gallery_retina_image_src', wp_get_attachment_image_src( $id, 'full' ), $id, $item, $data, $this->is_mobile );
				}
		...
		if ( ! isset( $image ) ) {
			return apply_filters( 'anugu_gallery_no_image_src', $item['link'], $id, $item, $data );
		}
		...
		// if ( ( $image_size != 'default' && ! $retina ) ) {
			// Return the image
			return apply_filters( 'anugu_gallery_image_src', $image, $id, $item, $data );
		}
		...
		if ( is_wp_error( $resized_image ) ) {
			...
			return apply_filters( 'anugu_gallery_image_src', $resized_image, $id, $item, $data );
		}
    ...
    public function get_toolbar_template( $data ) {
        ...
        $template  = '<div id="anugubox-buttons">';
            $template .= '<ul>';
                $template  = apply_filters( 'anugu_gallery_toolbar_start', $template, $data );

                // Prev
                $template .= '<li><a class="btnPrev" title="' . __( 'Previous', 'anugu-gallery-lite' ) . '" href="javascript:;"></a></li>';
                $template  = apply_filters( 'anugu_gallery_toolbar_after_prev', $template, $data );

                // Next
                $template .= '<li><a class="btnNext" title="' . __( 'Next', 'anugu-gallery-lite' ) . '" href="javascript:;"></a></li>';
                $template  = apply_filters( 'anugu_gallery_toolbar_after_next', $template, $data );

                // Title
                if ( $this->get_config( 'toolbar_title', $data ) ) {
                    $template .= '<li id="anugubox-buttons-title"><span>' . $this->get_config( 'title', $data ) . '</span></li>';
                    $template  = apply_filters( 'anugu_gallery_toolbar_after_title', $template, $data );
                }

                // Close
                $template .= '<li><a class="btnClose" title="' . __( 'Close', 'anugu-gallery-lite' ) . '" href="javascript:;"></a></li>';
                $template  = apply_filters( 'anugu_gallery_toolbar_after_close', $template, $data );

                $template  = apply_filters( 'anugu_gallery_toolbar_end', $template, $data );
            $template .= '</ul>';
        $template .= '</div>';

        // Return the template, filters applied and all.
        return apply_filters( 'anugu_gallery_toolbar', $template, $data );
    ...
    */
    public function get_lightbox_template( $data ) {
        ...
        $anugubox_wrap_css_classes = apply_filters( 'anugubox_wrap_css_classes', 'anugubox-wrap', $data );

        $anugubox_theme = apply_filters( 'anugubox_theme', 'anugubox-theme-' . $this->get_config( 'lightbox_theme', $data ), $data );

        $template = '<div class="' . $anugubox_wrap_css_classes . '" tabIndex="-1"><div class="anugubox-skin ' . $anugubox_theme . '"><div class="anugubox-outer"><div class="anugubox-inner">';

        // Top Left box
        $template .= '<div class="anugubox-position-overlay anugu-gallery-top-left">';
        $template  = apply_filters( 'anugubox_output_dynamic_position', $template, $data, 'top-left' );
        $template .= '</div>';

        // Top Right box
        $template .= '<div class="anugubox-position-overlay anugu-gallery-top-right">';
        $template  = apply_filters( 'anugubox_output_dynamic_position', $template, $data, 'top-right' );
        $template .= '</div>';

        // Bottom Left box
        $template .= '<div class="anugubox-position-overlay anugu-gallery-bottom-left">';
        $template  = apply_filters( 'anugubox_output_dynamic_position', $template, $data, 'bottom-left' );
        $template .= '</div>';

        // Bottom Right box
        $template .= '<div class="anugubox-position-overlay anugu-gallery-bottom-right">';
        $template  = apply_filters( 'anugubox_output_dynamic_position', $template, $data, 'bottom-right' );
        $template .= '</div>';

        $template .= '</div></div></div></div>';

        // Return the template, filters applied
        return apply_filters( 'anugu_gallery_lightbox_template', str_replace( "\n", '', $template ), $data );
    ...
    public function minify( $string, $stripDoubleForwardslashes = true ) {
        ...
        $stripDoubleForwardslashes = apply_filters( 'anugu_minify_strip_double_forward_slashes', $stripDoubleForwardslashes );
        ...
        return apply_filters( 'anugu_gallery_minified_string', $clean, $string );
     ...
     *
     * 1. Prevents Foobox from completely borking anugubox as if Foobox rules the world.
     *
     * @since 1.0.0
     */
    ...
    public function do_feed_output( $data ) {
        ...
        $gallery = '<div class="anugu-gallery-feed-output">';
            foreach ( $data['gallery'] as $id => $item ) {
                // Skip over images that are pending (ignore if in Preview mode).
                if ( isset( $item['status'] ) && 'pending' == $item['status'] && ! is_preview() ) {
                    continue;
                ...
                $imagesrc = $this->get_image_src( $id, $item, $data );
                $gallery .= '<img class="anugu-gallery-feed-image" src="' . esc_url( $imagesrc ) . '" title="' . trim( esc_html( $item['title'] ) ) . '" alt="' .trim( esc_html( $item['alt'] ) ) . '" />';
                break;
             }
        $gallery .= '</div>';

        return apply_filters( 'anugu_gallery_feed_output', $gallery, $data );
    ...
    public function get_indexable_images( $id ) {
        ...
        return apply_filters( 'anugu_gallery_indexable_images', $images, $this->index, $id );
     ...
     * @since 1.0.0
     *
     * @return object The Anugu_Gallery_Shortcode object.
     */
    public static function get_instance() {

        if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Anugu_Gallery_Shortcode ) ) {
            self::$instance = new Anugu_Gallery_Shortcode();
        }
...
$anugu_gallery_shortcode = Anugu_Gallery_Shortcode::get_instance();