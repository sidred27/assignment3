 ...
 */
class Anugu_Gallery_Common_Admin {
    ...
    /**
     * Handles any necessary DB upgrades for Anugu.
     *
     * @since 1.0.0
     */
    public function db_upgrade() {
        ...
        $captions = get_option( 'anugu_gallery_116' );
        if ( ! $captions ) {
            $galleries = Anugu_Gallery_Lite::get_instance()->_get_galleries();
            if ( $galleries ) {
                foreach ( $galleries as $gallery ) {
                    foreach ( (array) $gallery['gallery'] as $id => $item ) {
                        ...
                        Anugu_Gallery_Common::get_instance()->flush_gallery_caches( $gallery['id'], $gallery['config']['slug'] );
                    }
                }
            }

            update_option( 'anugu_gallery_116', true );
        }

        // 1.2.1: Convert all non-Anugu Post Type galleries into Anugu CPT galleries.
        $cptGalleries = get_option( 'anugu_gallery_121' );
        if ( ! $cptGalleries ) {
            ...
            $excludedPostTypes = array( 'anugu', 'anugu_album', 'attachment' );
            ...
            if ( count( $inPostGalleries->posts ) > 0 ) {
                ...
                foreach ( $inPostGalleries->posts as $post ) {
                    // Check if this is an Anugu or Anugu Album CPT
                    // If so, skip it
                    if ( $post->post_type == 'anugu' || $post->post_type == 'anugu_album' ) {
                        continue;
                    }
                    ...
                    // Some Posts save Anugu config data but don't have images - we don't want to migrate those,
                    // as we would end up with blank Anugu CPT galleries
                    if ( ! isset( $data['gallery'] ) || ! is_array( $data['gallery']) ) {
                        continue;
                    }

                    // If here, we need to create a new Anugu CPT
                    $cpt_args = array(
                    ...
                    $anuguGalleryID = wp_insert_post( $cpt_args );

                    // Check gallery creation was successful
                    if ( is_wp_error( $anuguGalleryID ) ) {
                        // @TODO how to handle errors?
                        continue;
                    }

                    // Get Anugu Gallery Post
                    $anuguPost = get_post( $anuguGalleryID );

                    // Map the title and slug of the post object to the custom fields if no value exists yet.
                    $data['config']['title'] = trim( strip_tags( $anuguPost->post_title ) );
                    $data['config']['slug']  = sanitize_text_field( $anuguPost->post_name );

                    // Store post metadata
                    update_post_meta( $anuguGalleryID, '_eg_gallery_data', $data );
                    update_post_meta( $anuguGalleryID, '_eg_in_gallery', $in );
                    update_post_meta( $anuguGalleryID, '_eg_gallery_old', $post->ID );
                    if ( ! empty( $data['config']['slug'] ) ) {
                        update_post_meta( $anuguGalleryID, '_eg_gallery_old_slug', $data['config']['slug'] );
                    }
                    ...
                    // Search for the anugu shortcode in the Post content, and change its ID to the new Anugu Gallery ID
                    if ( has_shortcode ( $post->post_content, 'anugu-gallery' ) ) {
                        $pattern = get_shortcode_regex();
                        if ( preg_match_all( '/'. $pattern .'/s', $post->post_content, $matches ) ) {
                            foreach ( $matches[2] as $key => $shortcode ) {
                                if ( $shortcode == 'anugu-gallery' ) {
                                    // Found an anugu-gallery shortcode
                                    // Change the ID
                                    $originalShortcode = $matches[0][ $key ];
                                    $replacementShortcode = str_replace( 'id="' . $post->ID . '"', 'id="' . $anuguGalleryID . '"', $originalShortcode );
                    ...
                    update_post_meta( $post->ID, '_eg_gallery_id', $anuguGalleryID );
            ...
            // Force the tags addon to convert any tags to the new CPT system for any galleries that have been converted to Anugu post type.
            delete_option( 'anugu_tags_taxonomy_migrated' );

            // Mark upgrade as complete
            update_option( 'anugu_gallery_121', true );
        }
    }
    /**
    ...
    public function notice_galleries_migrated() {
        ...
        <div class="notice updated">
            <p><?php _e( '<strong>Anugu Gallery:</strong> Your existing in-page Galleries can now be found by clicking on Anugu Gallery in the WordPress Admin menu.', 'anugu-gallery-lite' ); ?></p>
        </div>
        <?php
    ...
    /**
     * Loads styles for all Anugu-based Administration Screens.
     *
     * @since 1.3.1
     *
    ...
    public function admin_styles() {
        ...
        do_action( 'anugu_gallery_admin_styles' );
    ...
    public function admin_scripts() {
        ...
        wp_localize_script(
            ...
            array(
                ...
                'dismiss_notice_nonce'  => wp_create_nonce( 'anugu-gallery-dismiss-notice' ),
            )
        );
    ...
    /**
     * Deletes the Anugu gallery association for the image being deleted.
     *
     * @since 1.0.0
     *
    ...
    public function delete_gallery_association( $attach_id ) {
        ...
        // Only proceed if the image is attached to any Anugu galleries.
        if ( ! empty( $has_gallery ) ) {
            foreach ( (array) $has_gallery as $post_id ) {
                ...
                Anugu_Gallery_Common::get_instance()->flush_gallery_caches( $post_id, ( ! empty( $gallery_data['config']['slug'] ) ? $gallery_data['config']['slug'] : '' ) );
            }
        }
    ...
    public function trash_gallery( $id ) {
        ...
        Anugu_Gallery_Common::get_instance()->flush_gallery_caches( $id );

        // Return early if not an Anugu gallery.
        if ( 'anugu' !== $gallery->post_type ) {
            return;
        }
        ...
        do_action( 'anugu_gallery_trash', $id, $gallery_data );
    ...
    public function untrash_gallery( $id ) {
        ...
        do_action( 'anugu_gallery_untrash', $id, $gallery_data );
    ...
    */
     ...
     * - add_filter( 'anugu_gallery_shareasale_id', function() { return 1234; } );
     * - define( 'ANUGU_GALLERY_SHAREASALE_ID', 1234 );
     * - get_option( 'anugu_gallery_shareasale_id' ); (with the option being in the wp_options table)
     * 
     ...
     * ShareASale to then redirect to anugugallery.com/lite
     *
     * If no ID is present, just returns the anugugallery.com/lite URL with UTM tracking.
     *
     * @since 1.5.0
     */
    public function get_upgrade_link( $url = false, $medium = 'default', $button = 'default', $append = false ) {
        ...
        $shareasale_id = '';
        if ( defined( 'ANUGU_GALLERY_SHAREASALE_ID' ) ) {
            $shareasale_id = ANUGU_GALLERY_SHAREASALE_ID;
        }

        // If there's no constant, check if there's an option.
        if ( empty( $shareasale_id ) ) {
            $shareasale_id = get_option( 'anugu_gallery_shareasale_id', '' );
        }

        // Whether we have an ID or not, filter the ID.
        $shareasale_id = apply_filters( 'anugu_gallery_shareasale_id', $shareasale_id );
        ...
        if ( empty( $shareasale_id ) ) {
            ...
            $url = ( false !== $url ) ? trailingslashit( esc_url ( $url ) ) : 'https://anugugallery.com/lite/';
        ...
        return 'http://www.shareasale.com/r.cfm?u=' . $shareasale_id . '&b=566240&m=51693&afftrack=&urllink=anugugallery%2Ecom%2Flite%2F';
     ...
     * @since 1.0.0
     *
     * @return object The Anugu_Gallery_Common_Admin object.
     */
    public static function get_instance() {

        if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Anugu_Gallery_Common_Admin ) ) {
            self::$instance = new Anugu_Gallery_Common_Admin();
        }
...
$anugu_gallery_common_admin = Anugu_Gallery_Common_Admin::get_instance();