...
class Anugu_Gallery_License {
    ...
    public function __construct() {
        ...
        // Only attempt license verification, deactivation, refresh etc. if we're running Anugu Gallery.
    ...
    public function verify_key() {
        ...
        $verify = $this->perform_remote_request( 'verify-key', array( 'tgm-updater-key' => $_POST['anugu-license-key'] ) );

        // If it returns false, send back a generic error message and return.
        if ( ! $verify ) {
            $this->errors[] = __( 'There was an error connecting to the remote key API. Please try again later.', 'anugu-gallery-lite' );
            return;
        }
        ...
        $option                = get_option( 'anugu_gallery' );
        $option['key']         = $_POST['anugu-license-key'];
        $option['type']        = isset( $verify->type ) ? $verify->type : $option['type'];
        $option['is_expired']  = false;
        $option['is_disabled'] = false;
        $option['is_invalid']  = false;
        $this->success[]       = isset( $verify->success ) ? $verify->success : __( 'Congratulations! This site is now receiving automatic updates.', 'anugu-gallery-lite' );
        update_option( 'anugu_gallery', $option );
    ...
    public function is_verifying_key() {
        ...
        return isset( $_POST['anugu-license-key'] ) && isset( $_POST['anugu-gallery-verify-submit'] );
    ...
    public function verify_key_action() {
        ...
        return isset( $_POST['anugu-gallery-verify-submit'] ) && wp_verify_nonce( $_POST['anugu-gallery-key-nonce'], 'anugu-gallery-key-nonce' );
    ...
    public function maybe_validate_key() {
        ...
        if ( false === ( $validate = get_transient( '_eg_validate_license' ) ) ) {
            ...
            $timestamp = get_option( 'anugu_gallery_license_updates' );
            if ( ! $timestamp ) {
                $timestamp = strtotime( '+12 hours' );
                update_option( 'anugu_gallery_license_updates', $timestamp );
                $this->validate_key();
            } else {
                ...
                if ( $current_timestamp < $timestamp ) {
                    ...
                    update_option( 'anugu_gallery_license_updates', strtotime( '+12 hours' ) );
                    $this->validate_key();
                }
            }
        }
    ...
    public function validate_key( $forced = false ) {
        ...
        // If forced, set contextual success message.
        if ( $forced ) {
            $this->success[] = __( 'Congratulations! Your key has been refreshed successfully.', 'anugu-gallery-lite' );
        }
    ...
    public function deactivate_key() {
        ...
        $deactivate = $this->perform_remote_request( 'deactivate-key', array( 'tgm-updater-key' => $_POST['anugu-license-key'] ) );
        ...
        $this->success[] = isset( $deactivate->success ) ? $deactivate->success : __( 'Congratulations! You have deactivated the key from this site successfully.', 'anugu-gallery-lite' );
        update_option( 'anugu_gallery', Anugu_Gallery::default_options() );
    ...
    public function is_deactivating_key() {
        ...
        return isset( $_POST['anugu-license-key'] ) && isset( $_POST['anugu-gallery-deactivate-submit'] );
    ...
    public function deactivate_key_action() {
        ...
        return isset( $_POST['anugu-gallery-deactivate-submit'] ) && wp_verify_nonce( $_POST['anugu-gallery-key-nonce'], 'anugu-gallery-key-nonce' );
    ...
    public function is_refreshing_key() {
        ...
        return isset( $_POST['anugu-license-key'] ) && isset( $_POST['anugu-gallery-refresh-submit'] );
    ...
    public function refresh_key_action() {
        ...
        return isset( $_POST['anugu-gallery-refresh-submit'] ) && wp_verify_nonce( $_POST['anugu-gallery-key-nonce'], 'anugu-gallery-key-nonce' );
    ...
    public function notices() {
        ...
        ?>
        <div class="error">
            <p><?php printf( __( 'No valid license key has been entered, so automatic updates for Anugu Gallery have been turned off. <a href="%s">Please click here to enter your license key and begin receiving automatic updates.</a>', 'anugu-gallery-lite' ), esc_url( add_query_arg( array( 'post_type' => 'anugu', 'page' => 'anugu-gallery-settings' ), admin_url( 'edit.php' ) ) ) ); ?></p>
        </div>
        <?php
        endif;
        ...
        ?>
        <div class="error">
            <p><?php printf( __( 'Your license key for Anugu Gallery has expired. <a href="%s" target="_blank">Please click here to renew your license key and continue receiving automatic updates.</a>', 'anugu-gallery-lite' ), 'https://anugugallery.com/login/' ); ?></p>
        </div>
        <?php
        endif;
        ...
        ?>
        <div class="error">
            <p><?php _e( 'Your license key for Anugu Gallery has been disabled. Please use a different key to continue receiving automatic updates.', 'anugu-gallery-lite' ); ?></p>
        </div>
        <?php
        endif;
        ...
        ?>
        <div class="error">
            <p><?php _e( 'Your license key for Anugu Gallery is invalid. The key no longer exists or the user associated with the key has been deleted. Please use a different key to continue receiving automatic updates.', 'anugu-gallery-lite' ); ?></p>
        </div>
        <?php
        endif;
    ...
    public function perform_remote_request( $action, $body = array(), $headers = array(), $return_format = 'json' ) {
        ...
        $response      = wp_remote_post( 'http://anugugallery.com', $post );
     ...
     * @since 1.0.0
     *
     * @return object The Anugu_Gallery_License object.
     */
    public static function get_instance() {

        if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Anugu_Gallery_License ) ) {
            self::$instance = new Anugu_Gallery_License();
        }
...
$anugu_gallery_license = Anugu_Gallery_License::get_instance();