...
class Anugu_Gutenberg {
	...
	public function block_assets() {
		...
		wp_enqueue_style(
			'anugu_gutenberg-style-css', // Handle.
	...
	public function get_galleries() {
		...
		if ( ! is_null( $current_screen ) && method_exists( $current_screen, 'is_block_editor' ) && $current_screen->is_block_editor() ) {
			$request = new WP_REST_Request( 'GET', '/wp/v2/anugu-gallery' );
	...
	public function editor_assets() {
		...
		wp_enqueue_script(
			'anugu_gutenberg-block-js',
			plugins_url( 'assets/js/anugu-gutenberg.js', $this->base->file ), // Block.build.js: we register the block here and built with Webpack.
    ...
    public static function get_instance() {
        ...
        if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Anugu_Gutenberg ) ) {
...
$anugu_gutenberg = Anugu_Gutenberg::get_instance();