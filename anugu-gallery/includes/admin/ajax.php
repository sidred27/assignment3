<?php
/**
 * Handles all admin ajax interactions for the Anugu Gallery plugin.
 *
 * @since 1.0.0
 *
...
add_action( 'wp_ajax_anugu_gallery_change_type', 'anugu_gallery_ajax_change_type' );
/**
 * Changes the type of gallery to the user selection.
 *
 * @since 1.0.0
 */
function anugu_gallery_ajax_change_type() {

    // Run a security check first.
    check_admin_referer( 'anugu-gallery-change-type', 'nonce' );
    ...
    ob_start();
    $instance = Anugu_Gallery_Metaboxes::get_instance();
...
add_action( 'wp_ajax_anugu_gallery_change_preview', 'anugu_gallery_ajax_change_preview' );
/**
 * Returns the output for the Preview Metabox for the given Gallery Type.
 *
 * @since 1.5.0
 */
function anugu_gallery_ajax_change_preview() {

    // Run a security check first.
    check_admin_referer( 'anugu-gallery-change-preview', 'nonce' );
    ...
    $data    = Anugu_Gallery_Lite::get_instance()->get_gallery( $post_id );
    ...
    foreach ( $_POST['data'] as $index => $field ) {
        ...
        if ( strpos( $field['name'], '_anugu_gallery[' ) === false ) {
            continue;
        }
...
add_action( 'wp_ajax_anugu_gallery_set_user_setting', 'anugu_gallery_ajax_set_user_setting' );
/**
 * Stores a user setting for the logged in WordPress User
 *
 * @since 1.5.0
 */
function anugu_gallery_ajax_set_user_setting() {

    // Run a security check first.
    check_admin_referer( 'anugu-gallery-set-user-setting', 'nonce' );
...
add_action( 'wp_ajax_anugu_gallery_load_image', 'anugu_gallery_ajax_load_image' );
/**
 * Loads an image into a gallery.
 *
 * @since 1.0.0
 */
function anugu_gallery_ajax_load_image() {

    // Run a security check first.
    check_admin_referer( 'anugu-gallery-load-image', 'nonce' );

    // Prepare variables.
    $id      = absint( $_POST['id'] );
    $post_id = absint( $_POST['post_id'] );

    // Set post meta to show that this image is attached to one or more Anugu galleries.
    ...
    $gallery_data = anugu_gallery_ajax_prepare_gallery_data( $gallery_data, $id );
    update_post_meta( $post_id, '_eg_gallery_data', $gallery_data );

    // Run hook before building out the item.
    do_action( 'anugu_gallery_ajax_load_image', $id, $post_id );

    // Build out the individual HTML output for the gallery image that has just been uploaded.
    $html = Anugu_Gallery_Metaboxes::get_instance()->get_gallery_item( $id, $gallery_data['gallery'][$id], $post_id );

    // Allow addons to filter the HTML output
    $html = apply_filters( 'anugu_gallery_ajax_get_gallery_item_html', $html, $gallery_data, $id, $post_id );

    // Flush the gallery cache.
    Anugu_Gallery_Common::get_instance()->flush_gallery_caches( $post_id );
...
add_action( 'wp_ajax_anugu_gallery_insert_images', 'anugu_gallery_ajax_insert_images' );
/**
 * Inserts one or more images from the Media Library into a gallery.
 *
 * @since 1.0.0
 */
function anugu_gallery_ajax_insert_images() {

    // Run a security check first.
    check_admin_referer( 'anugu-gallery-insert-images', 'nonce' );
    ...
    foreach ( (array) $images as $i => $image ) {
        ...
        $in_gallery[] = $image['id'];
        $gallery_data = anugu_gallery_ajax_prepare_gallery_data( $gallery_data, $image['id'], $image );
    }
    ...
    do_action( 'anugu_gallery_ajax_insert_images', $images, $post_id );
    ...
    foreach ( (array) $gallery_data['gallery'] as $id => $data ) {
        $html .= Anugu_Gallery_Metaboxes::get_instance()->get_gallery_item( $id, $data, $post_id );
    }
...
add_action( 'wp_ajax_anugu_gallery_sort_images', 'anugu_gallery_ajax_sort_images' );
/**
 * Sorts images based on user-dragged position in the gallery.
 *
 * @since 1.0.0
 */
function anugu_gallery_ajax_sort_images() {

    // Run a security check first.
    check_admin_referer( 'anugu-gallery-sort', 'nonce' );
...
add_action( 'wp_ajax_anugu_gallery_remove_image', 'anugu_gallery_ajax_remove_image' );
/**
 * Removes an image from a gallery.
 *
 * @since 1.0.0
 */
function anugu_gallery_ajax_remove_image() {

    // Run a security check first.
    check_admin_referer( 'anugu-gallery-remove-image', 'nonce' );
    ...
    do_action( 'anugu_gallery_ajax_remove_image', $attach_id, $post_id );
...
add_action( 'wp_ajax_anugu_gallery_remove_images', 'anugu_gallery_ajax_remove_images' );
/**
 * Removes multiple images from a gallery.
 *
 * @since 1.3.2.4
 */
function anugu_gallery_ajax_remove_images() {
    ...
    // Run hook before finishing the reponse.
    do_action( 'anugu_gallery_ajax_remove_images', $attach_id, $post_id );
...
add_action( 'wp_ajax_anugu_gallery_save_meta', 'anugu_gallery_ajax_save_meta' );
/**
 * Saves the metadata for an image in a gallery.
 *
 * @since 1.0.0
 */
function anugu_gallery_ajax_save_meta() {

    // Run a security check first.
    check_admin_referer( 'anugu-gallery-save-meta', 'nonce' );
    ...
    $gallery_data = apply_filters( 'anugu_gallery_ajax_save_meta', $gallery_data, $meta, $attach_id, $post_id );
...
add_action( 'wp_ajax_anugu_gallery_save_bulk_meta', 'anugu_gallery_ajax_save_bulk_meta' );
/**
 * Saves the metadata for multiple images in a gallery (bulk edit).
 *
 * @since 1.4.2.2
 */
function anugu_gallery_ajax_save_bulk_meta() {
    ...
    foreach ( $image_ids as $image_id ) {
        ...
        // Allow filtering of meta before saving.
        $gallery_data = apply_filters( 'anugu_gallery_ajax_save_bulk_meta', $gallery_data, $meta, $image_id, $post_id );
    }
...
add_action( 'wp_ajax_anugu_gallery_refresh', 'anugu_gallery_ajax_refresh' );
/**
 * Refreshes the DOM view for a gallery.
 *
 * @since 1.0.0
 */
function anugu_gallery_ajax_refresh() {

    // Run a security check first.
    check_admin_referer( 'anugu-gallery-refresh', 'nonce' );
    ...
    foreach ( (array) $gallery_data['gallery'] as $id => $data ) {
        $gallery .= Anugu_Gallery_Metaboxes::get_instance()->get_gallery_item( $id, $data, $post_id );
    }
...
add_action( 'wp_ajax_anugu_gallery_load_gallery_data', 'anugu_gallery_ajax_load_gallery_data' );
/**
 * Retrieves and return gallery data for the specified ID.
 *
 * @since 1.0.0
 */
function anugu_gallery_ajax_load_gallery_data() {
...
add_action( 'wp_ajax_anugu_gallery_install_addon', 'anugu_gallery_ajax_install_addon' );
/**
 * Installs an Anugu addon.
 *
 * @since 1.0.0
 */
function anugu_gallery_ajax_install_addon() {

    // Run a security check first.
    check_admin_referer( 'anugu-gallery-install', 'nonce' );
    ...
    if ( isset( $_POST['plugin'] ) ) {
        ...
        require_once plugin_dir_path( Anugu_Gallery::get_instance()->file ) . 'includes/admin/skin.php';

        // Create the plugin upgrader with our custom skin.
        $installer = new Plugin_Upgrader( $skin = new Anugu_Gallery_Skin() );
...
add_action( 'wp_ajax_anugu_gallery_activate_addon', 'anugu_gallery_ajax_activate_addon' );
/**
 * Activates an Anugu addon.
 *
 * @since 1.0.0
 */
function anugu_gallery_ajax_activate_addon() {

    // Run a security check first.
    check_admin_referer( 'anugu-gallery-activate', 'nonce' );
...
add_action( 'wp_ajax_anugu_gallery_deactivate_addon', 'anugu_gallery_ajax_deactivate_addon' );
/**
 * Deactivates an Anugu addon.
 *
 * @since 1.0.0
 */
function anugu_gallery_ajax_deactivate_addon() {

    // Run a security check first.
    check_admin_referer( 'anugu-gallery-deactivate', 'nonce' );
 ...
 */
function anugu_gallery_ajax_prepare_gallery_data( $gallery_data, $id, $image = false ) {
    ...
    $image = apply_filters( 'anugu_gallery_ajax_prepare_gallery_data_item', $new_image, $image, $id, $gallery_data );
    ...
    // Filter and return
    $gallery_data = apply_filters( 'anugu_gallery_ajax_item_data', $gallery_data, $attachment, $id, $image );
...
/**
 * Called whenever a notice is dismissed in Anugu Gallery or its Addons.
 *
 ...
 * @since 1.3.5
 */
function anugu_gallery_ajax_dismiss_notice() {

    // Run a security check first.
    check_admin_referer( 'anugu-gallery-dismiss-notice', 'nonce' );

    // Deactivate the notice
    if ( isset( $_POST['notice'] ) ) {
        // Init the notice class and mark notice as deactivated
        $notice = Anugu_Gallery_Notice_Admin::get_instance();
...
add_action( 'wp_ajax_anugu_gallery_ajax_dismiss_notice', 'anugu_gallery_ajax_dismiss_notice' );
 ...
 * @since 1.4.1.4
 */
add_action( 'wp_ajax_anugu_gallery_get_attachment_links', 'anugu_gallery_get_attachment_links' );
function anugu_gallery_get_attachment_links() {

    // Check nonce
    check_ajax_referer( 'anugu-gallery-save-meta', 'nonce' );
 ...
 * @since 1.5.0
 */
add_action( 'wp_ajax_anugu_gallery_editor_get_galleries', 'anugu_gallery_editor_get_galleries' );
function anugu_gallery_editor_get_galleries() {

    // Check nonce
    check_admin_referer( 'anugu-gallery-editor-get-galleries', 'nonce' );
    ...
    // Get galleries
    $instance = Anugu_Gallery_Lite::get_instance();
 ...
 * @since 1.5.0.3
 */
add_action( 'wp_ajax_anugu_gallery_move_media', 'anugu_gallery_move_media' );
function anugu_gallery_move_media() {

    // Check nonce
    check_admin_referer( 'anugu-gallery-move-media', 'nonce' );
    ...
    if ( ! $from_gallery_id ) {
        wp_send_json_error( __( 'The From Gallery ID has not been specified.', 'anugu-gallery-lite' ) );
    }
    if ( ! $to_gallery_id ) {
        wp_send_json_error( __( 'The From Gallery ID has not been specified.', 'anugu-gallery-lite' ) );
    }
    if ( count( $image_ids ) == 0 ) {
        wp_send_json_error( __( 'No images were selected to be moved between Galleries.', 'anugu-gallery-lite' ) );
    }

    // Get from and to Galleries
    $from_gallery   = Anugu_Gallery::get_instance()->get_gallery( $from_gallery_id );
    $to_gallery     = Anugu_Gallery::get_instance()->get_gallery( $to_gallery_id );
