...
class Anugu_Gallery_Editor {
    ...
    public function media_button( $buttons ) {
        ...
        wp_localize_script( $this->base->plugin_slug . '-gallery-select-script', 'anugu_gallery_editor', array(
            'modal_title'           => __( 'Insert', 'anugu-gallery-lite' ),
            'insert_button_label'   => __( 'Insert', 'anugu-gallery-lite' ),
        ) );

        // Create the media button.
        $button = '<a id="anugu-media-modal-button" href="#" class="button anugu-gallery-choose-gallery" data-action="gallery" title="' . esc_attr__( 'Add Gallery', 'anugu-gallery-lite' ) . '" >
            <span class="anugu-media-icon"></span> ' .
             __( 'Add Gallery', 'anugu-gallery-lite' ) . 
        '</a>';

        // Filter the button.
        $button = apply_filters( 'anugu_gallery_media_button', $button, $buttons );
    ...
    /**
     * Checks for the existience of any Anugu Gallery shortcodes in the Post's content,
     * storing this Post's ID in each Anugu Gallery.
     *
     * This allows Anugu's WP_List_Table to tell the user which Post(s) the Gallery is
     * included in.
     *
     * @since 1.3.3.6
     *
    ...
    /**
     * Removes the given Post ID from all Anugu Galleries that contain the Post ID
     *
     * @since 1.3.3.6
     *
     * @param int $post_id Post ID
    ...
    /**
     * Checks for Anugu Gallery shortcodes in the given content.
     *
    ...
    private function update_gallery_post_ids( $post_id, $remove = false ) {
        ...
        if ( in_array( $post->post_type, array( 'anugu', 'anugu_album' ) ) ) {
            return;
        }
        ...
        // Content has Anugu shortcode(s)
        // Extract them to get Gallery IDs
        $pattern = '\[(\[?)(anugu\-gallery)(?![\w-])([^\]\/]*(?:\/(?!\])[^\]\/]*)*?)(?:(\/)\]|\](?:([^\[]*+(?:\[(?!\/\2\])[^\[]*+)*+)\[\/\2\])?)(\]?)';
     ...
     * @since 1.0.0
     *
     * @return object The Anugu_Gallery_Editor object.
     */
    public static function get_instance() {

        if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Anugu_Gallery_Editor ) ) {
            self::$instance = new Anugu_Gallery_Editor();
        }
...
$anugu_gallery_editor = Anugu_Gallery_Editor::get_instance();