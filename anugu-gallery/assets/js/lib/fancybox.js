...
(function(B) {
    var L, T, Q, M, d, m, J, A, O, z, C = 0,
        ...
        r = function() {
            ...
            H.width = "auto";
            H.height = "auto";
            L.html('<p id="anugubox-error">The requested content cannot be loaded.<br />Please try again later.</p>');
            n()
        }, w = function() {
            var Z = j[C],
                W, Y, ab, aa, V, X;
            r();
            H = B.extend({}, B.fn.anugubox.defaults, (typeof B(Z).data("anugubox") == "undefined" ? H : B(Z).data("anugubox")));
            X = H.onStart(j, C, H);
            if (X === false) {
                P = false;
                return
            ...
            /* is href/W a link? */
            if ( typeof W === 'string' && B.anugubox.isUrl(W) && !B.anugubox.isImage(W) ) {
                document.getElementById("anugubox-loading").style.visibility = "hidden";
            ...
            B(".anugubox-inline-tmp").unbind("anugubox-cancel").bind("anugubox-change", function() {
            ...
            switch (Y) {
                case "html":
                    ...
                    break;
                case "inline":
                    if (B(Z).parent().is("#anugubox-content") === true) {
                        P = false;
                        return
                    }
                    B('<div class="anugubox-inline-tmp" />').hide().insertBefore(B(Z)).bind("anugubox-cleanup", function() {
                        B(this).replaceWith(m.children())
                    }).bind("anugubox-cancel", function() {
            ...
            if (M.is(":visible") && false === G.onCleanup(y, e, G)) {
                B.event.trigger("anugubox-cancel");
                P = false;
                return
            }
            P = true;
            ...
            if (G.overlayShow) {
                ...
                if (!Q.is(":visible")) {
                    if (S) {
                        B("select:not(#anugubox-tmp select)").filter(function() {
                            return this.style.visibility !== "hidden"
                        }).css({
                            visibility: "hidden"
                        }).one("anugubox-cleanup", function() {
            ...
            if (M.is(":visible")) {
                ...
                m.fadeTo(G.changeFade, 0.3, function() {
                    ...
                    B.event.trigger("anugubox-change");
            ...
            if (V && V.length) {
                if (G.titlePosition == "float") {
                    return '<table id="anugubox-title-float-wrap" cellpadding="0" cellspacing="0"><tr><td id="anugubox-title-float-left"></td><td id="anugubox-title-float-main">' + B.decodeEntities(V) + '</td><td id="anugubox-title-float-right"></td></tr></table>'
                }
                return '<div id="anugubox-title-' + G.titlePosition + '">' + B.decodeEntities(V) + "</div>"
            }
            return false
        }, l = function() {
            t = G.title || "";
            ...
            A.addClass("anugubox-title-" + G.titlePosition).html(t).appendTo("body").show();
                ...
                B(document).bind("keydown.fb", function(V) {
                    if (V.keyCode == 27 && G.enableEscapeButton) {
                        ...
                        if ((V.keyCode == 37 || V.keyCode == 39) && G.enableKeyboardNav && V.target.tagName !== "INPUT" && V.target.tagName !== "TEXTAREA" && V.target.tagName !== "SELECT") {
                            V.preventDefault();
                            B.anugubox[V.keyCode == 37 ? "prev" : "next"]()
                        }
                    }
                })
            }
            ...
            if (G.hideOnContentClick) {
                m.bind("click", B.anugubox.close)
            }
            if (G.hideOnOverlayClick) {
                Q.bind("click", B.anugubox.close)
            }
            B(window).bind("resize.fb", B.anugubox.resize);
            if (G.centerOnScroll) {
                B(window).bind("scroll.fb", B.anugubox.center)
            }
            if (G.type == "iframe") {
                B('<iframe id="anugubox-frame" name="anugubox-frame' + new Date().getTime() + '" frameborder="0" hspace="0" scrolling="' + H.scrolling + '" src="' + G.href + '"></iframe>').appendTo(m)
            }
            M.show();
            P = false;
            B.anugubox.center();
    ...
    B.fn.anugubox = function(V) {
        if (!B(this).length) {
            return this
        }
        B(this).data("anugubox", B.extend({}, V, (B.metadata ? B(this).metadata() : {}))).unbind("click.fb").bind("click.fb", function(X) {
            X.preventDefault();
            if (P) {
                return
            }
        ...
        if (B.isArray(Y)) {
            for (var W = 0, V = Y.length; W < V; W++) {
                if (typeof Y[W] == "object") {
                    B(Y[W]).data("anugubox", B.extend({}, X, Y[W]))
                } else {
                    Y[W] = B({}).data("anugubox", B.extend({
                        content: Y[W]
                    }, X))
                }
            }
            ...
            if (typeof Y == "object") {
                B(Y).data("anugubox", B.extend({}, X, Y))
            } else {
                Y = B({}).data("anugubox", B.extend({
                    content: Y
                }, X))
            }
            j.push(Y)
    ...
    B.anugubox.showActivity = function() {
        clearInterval(p);
        T.show();
        p = setInterval(a, 66)
    };
    B.anugubox.hideActivity = function() {
        T.hide()
    };
    B.anugubox.next = function() {
        return B.anugubox.pos(e + 1)
    };
    B.anugubox.prev = function() {
        return B.anugubox.pos(e - 1)
    };
    B.anugubox.pos = function(V) {
        if (P) {
            return
        }
        V = parseInt(V);
    ...
    B.anugubox.close = function() {
        ...
        function V() {
            ...
            A.empty().hide();
            M.hide();
            B.event.trigger("anugubox-cleanup");
            m.empty();
            G.onClosed(y, e, G);
            y = H = [];
            e = C = 0;
    ...
    B.anugubox.resize = function() {
        ...
        M.css(c);
        B.anugubox.center(true)
    };
    B.anugubox.center = function() {
        var V, W;
        if (P) {
            return
        }
    ...
    B.anugubox.init = function() {
        if (B("#anugubox-wrap").length) {
            return
        }
        B("body").append(L = B('<div id="anugubox-tmp"></div>'), T = B('<div id="anugubox-loading"><div></div></div>'), Q = B('<div id="anugubox-overlay"></div>'), M = B('<div id="anugubox-wrap"></div>'));
        d = B('<div id="anugubox-outer"></div>').append('<div class="anugubox-bg" id="anugubox-bg-n"></div><div class="anugubox-bg" id="anugubox-bg-ne"></div><div class="anugubox-bg" id="anugubox-bg-e"></div><div class="anugubox-bg" id="anugubox-bg-se"></div><div class="anugubox-bg" id="anugubox-bg-s"></div><div class="anugubox-bg" id="anugubox-bg-sw"></div><div class="anugubox-bg" id="anugubox-bg-w"></div><div class="anugubox-bg" id="anugubox-bg-nw"></div>').appendTo(M);
        d.append(m = B('<div id="anugubox-content"></div>'), J = B('<a id="anugubox-close"></a>'), A = B('<div id="anugubox-title"></div>'), O = B('<a href="javascript:;" id="anugubox-left"><span class="fancy-ico" id="anugubox-left-ico"></span></a>'), z = B('<a href="javascript:;" id="anugubox-right"><span class="fancy-ico" id="anugubox-right-ico"></span></a>'));
        J.click(B.anugubox.close);
        T.click(B.anugubox.cancel);
        O.click(function(V) {
            V.preventDefault();
            B.anugubox.prev()
        });
            ...
            M.bind("mousewheel.fb", function(V, W) {
                if (P) {
                    ...
                    if (B(V.target).get(0).clientHeight == 0 || B(V.target).get(0).scrollHeight === B(V.target).get(0).clientHeight) {
                        V.preventDefault();
                        B.anugubox[W > 0 ? "prev" : "next"]()
                    }
                }
            })
        }
        if (!B.support.opacity) {
            M.addClass("anugubox-ie")
        }
        if (S) {
            T.addClass("anugubox-ie6");
            M.addClass("anugubox-ie6");
            B('<iframe id="anugubox-hide-sel-frame" src="' + (/^https/i.test(window.location.href || "") ? "javascript:void(false)" : "about:blank") + '" scrolling="no" border="0" frameborder="0" tabindex="-1"></iframe>').prependTo(d)
        }
    };
    B.anugubox.isUrl = function(str) {
        return str.match(/^(https?):\/\/((?:[a-z0-9.-]|%[0-9A-F]{2}){3,})(?::(\d+))?((?:\/(?:[a-z0-9-._~!$&'()*+,;=:@]|%[0-9A-F]{2})*)*)(?:\?((?:[a-z0-9-._~!$&'()*+,;=:\/?@]|%[0-9A-F]{2})*))?(?:#((?:[a-z0-9-._~!$&'()*+,;=:\/?@]|%[0-9A-F]{2})*))?$/i);
    };
    B.anugubox.isImage = function(str) {
