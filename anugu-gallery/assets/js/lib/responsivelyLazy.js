...
if ( typeof anugu_lazy_load !== 'undefined' && anugu_lazy_load == 'true' ) {
...
window.responsivelyLazy = (function () {
    ...
    var updateElement = function (container, element) {
        var options = element.getAttribute('data-anugu-srcset');
        if (options !== null) {
        ...
        if (bestSelectedOption === null) {
            bestSelectedOption = [element.getAttribute('data-anugu-src'), 999999];
        }
        ...
        if (container.lastSetOption[1] < bestSelectedOption[1]) {
            ...
            image.onload = function () {
				...
				if ( container.getAttribute( 'class' ) == 'anugu-lazy' && jQuery( container ).not( "img" ) ) {
					...
					var gallery_id    = jQuery( the_image ).data( 'anugu-gallery-id' );
					var item_id       = jQuery( container ).data( 'anugu-item-id' );
					...
					var gallery_id    = jQuery( container ).data( 'anugu-gallery-id' );
					var item_id       = jQuery( container ).data( 'anugu-item-id' );
					var naturalWidth  = this.naturalWidth;
					var naturalHeight = this.naturalHeight;
				...
				jQuery( document ).trigger(
					{
						type:           'anugu_image_lazy_load_complete',
    ...
    var run = function ( galleryClass ) {
        ...
        if ( galleryClass ) {
            ...
            if ( anugu_lazy_load_delay === 'undefined' || anugu_lazy_load_initial === false || anugu_lazy_load_initial === 'undefined' ) {
                /* if we can't locate these vars, at least define the delay - there is no delay, super fast */
                anugu_lazy_load_delay = 0;
            }


            myVar = setTimeout(function () {

                if ( jQuery( galleryClass + ' .anugu-lazy > img').exists() ) {
                    //console.log('exists');
                    update(document.querySelectorAll(galleryClass + ' .anugu-lazy > img'), false);
                } else if ( jQuery(  galleryClass + ' img.anugu-lazy').exists() ) {
                    //console.log('exists');
                    update(document.querySelectorAll(galleryClass + ' img.anugu-lazy'), true);
                }

                anugu_lazy_load_initial == true; // ok, we did the initial load so now delay can happen

            }, anugu_lazy_load_delay);




        }
        //update(document.querySelectorAll('img.anugu-lazy'), true);
    };
        ...
        image.onload = image.onerror = function () {
            ...
            if (hasIntersectionObserverSupport) {
                ...
                var updateIntersectionObservers = function () {
                    var elements = document.querySelectorAll('.anugu-lazy');
                    var elementsCount = elements.length;
                    for (var i = 0; i < elementsCount; i++) {
                        var element = elements[i];
