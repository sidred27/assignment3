...
* Image Model
*/
var AnuguGalleryImage = Backbone.Model.extend( {
...
/**
* Images Collection
* - Comprises of all images in an Anugu Gallery
* - Each image is represented by an AnuguGalleryImage Model
*/
var AnuguGalleryImages = new Backbone.Collection;

/**
* Modal Window
* - Used by most Anugu Backbone views to display information e.g. bulk edit, edit single image etc.
*/
if ( typeof AnuguGalleryModalWindow == 'undefined' ) {
    var AnuguGalleryModalWindow = new wp.media.view.Modal( {
        controller: {
            trigger: function() {
            }
        }
...
* View
*/
var AnuguGalleryEditView = wp.Backbone.View.extend( {
    ...
    */
    template:   wp.template( 'anugu-meta-editor' ),
    ...
    */
    events: {
        ...
        'click .actions a.anugu-gallery-meta-submit':  'saveItem',
    ...
    * Initialize
    *
    * @param object model   AnuguGalleryImage Backbone Model
    */
    ...
    render: function() {
        ...
        if ( this.child_views.length > 0 ) {
            this.child_views.forEach( function( view ) {
                ...
                this.$el.find( 'div.anugu-addons' ).append( child_view.render().el );
            }, this );
        }
    ...
    * wp.media.view.AnuguGalleryError
    */
    renderError: function( error ) {
        ...
        // Define view
        var view = new wp.media.view.AnuguGalleryError( {
            model: model
        } );
    ...
    */
    saveItem: function( event ) {
        ...
        wp.media.ajax( 'anugu_gallery_save_meta', {
            context: this,
            data: {
                nonce:     anugu_gallery_metabox.save_nonce,
            ...
            success: function( response ) {
                ...
                var item = JSON.stringify( this.model.attributes ),
                    item_element = jQuery( 'ul#anugu-gallery-output li#' + this.model.get( 'id' ) );

                // Assign the JSON
                jQuery( item_element ).attr( 'data-anugu-gallery-image-model', item );
    ...
    */
    insertMediaFileLink: function( event ) {
        ...
        wp.media.ajax( 'anugu_gallery_get_attachment_links', {
            context: this,
            data: {
                nonce:          anugu_gallery_metabox.save_nonce,
...
jQuery( document ).ready( function( $ ) {
    ...
    // Edit Image
    $( document ).on( 'click', '#anugu-gallery-main a.anugu-gallery-modify-image', function( e ) {
        ...
        var attachment_id = $( this ).parent().data( 'anugu-gallery-image' );

        // Pass the collection of images for this gallery to the modal view, as well
        // as the selected attachment
        AnuguGalleryModalWindow.content( new AnuguGalleryEditView( {
            collection:     AnuguGalleryImages,
            child_views:    AnuguGalleryChildViews,
...
/**
* Populates the AnuguGalleryImages Backbone collection, which comprises of a set of Anugu Gallery Images
*
* Called when images are added, deleted, reordered or selected
*
* @global           AnuguGalleryImages     The backbone collection of images
* @param    bool    selected_only           Only populate collection with images the user has selected
*/
function AnuguGalleryImagesUpdate( selected_only ) {

    // Clear the collection
    AnuguGalleryImages.reset();

    // Iterate through the gallery images in the DOM, adding them to the collection
    var selector = 'ul#anugu-gallery-output li.anugu-gallery-image' + ( selected_only ? '.selected' : '' );

    jQuery( selector ).each( function() {
        // Build an AnuguGalleryImage Backbone Model from the JSON supplied in the element
        var anugu_gallery_image = jQuery.parseJSON( jQuery( this ).attr( 'data-anugu-gallery-image-model' ) );

        // Strip slashes from some fields
        anugu_gallery_image.alt = AnuguGalleryStripslashes( anugu_gallery_image.alt );

        // Add the model to the collection
        AnuguGalleryImages.add( new AnuguGalleryImage( anugu_gallery_image ) );
    } );

    // Update the count in the UI
    jQuery( '#anugu-gallery-main span.count' ).text( jQuery( 'ul#anugu-gallery-output li.anugu-gallery-image' ).length );
...
*/
function AnuguGalleryStripslashes( str ) {
